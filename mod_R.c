...
static int RApacheInfo()
{
   ...
   EXEC("scrub <- function(st){ if (typeof(st) == 'closure') { st <-'<u>typeof closure</u>'; return(st); }; if (is.null(st)) return('null'); if (is.na(st)) return('NA'); if (length(st) == 0) return ('length 0 sting'); st  <- as.character(st)  ; st <- gsub('&','&amp;',st); st <- gsub('@','_at_',st); st <- gsub('<','&lt;',st); st <- gsub('>','&gt;',st); if (length(st) == 0 || is.null(st) || st == '') st <- '&nbsp;'; st }");
   ...
   PUTS("<pre>");
   PUTS("Copyright 2011-2020 Vanderbilt University (update for R-3.6.3)\n");
   PUTS("\n");
